function [RF_DATA, HEADER, transducer_code,flag]=import_RFdata2MATLAB_pane_mod(filename)
...
fid=fopen([filename]);
number_of_header_els=3;                                                    %% Number of header parameters for each frame 
...
header.Length_of_RF_row=A(1);                                              %% Number of RF signal samples (1 symbol in data file)
header.Number_of_RF_rows=A(2);                                             %% Number of RF signal rows in a predefined RF Window
header.Number_of_frames=A(3);                                              %% Number of frames in the file
...
frame_to_reshape=((fread(fid,2*size_of_frame,'int16')));                     %% Read single RF frame vector (int16 data type)
%% RF stream reshaped to matrix (all the collected full frames)
RF_DATA{i}=reshape(frame_to_reshape,2,header.Length_of_RF_row,header.Number_of_RF_rows);  

size_in_bytes_run=(number_of_header_els*4)+(0*header.Number_of_RF_rows*4)+2*(size_of_frame*2);
